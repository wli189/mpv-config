...
local o = {
    enabled = true,
    config_path = "~~/trakt-config.json",
}

options.read_options(o, _, function() end)

local state = {}
local config_file = mp.command_native({ "expand-path", o.config_path })
...
local function query_search_show(name, season, episode, config)
    ...
    mp.osd_message("Found on trakt.tv: " .. state.title .. " S" .. season .. "E" .. episode)
...
local function query_movie(movie, year, config)
    ...
    for _, item in ipairs(res) do
        if item.movie.year == tonumber(year) then
            ...
            mp.osd_message("Found: " .. state.title)
            return
        end
    end
    msg.error("No matching movie found")
end
